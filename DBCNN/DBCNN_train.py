...
class DBCNNManager(object):
    def __init__(self, options, path):
        ...
        if self._options['dataset'] == 'livec':
            ...
            val_data = LIVEChallengeFolder.LIVEChallengeFolder(
                    root=self._path['livec'], loader = default_loader, index = self._options['val_index'],
            ...
            raise AttributeError('Only support LIVEC right now!')
        ...
        self._test_loader = torch.utils.data.DataLoader(
            ...
            shuffle=False, num_workers=1, pin_memory=True) #num_workers=0
    ...
    def train(self):
        ...
        for t in range(self._options['epochs']):
            ...
            # test_srcc, test_plcc, test_krocc = self._consitency(self._test_loader)
            test_srcc, test_plcc, test_krocc = self._consitency(self._val_loader)
            if test_srcc > best_srcc:
                ...
                pwd = os.getcwd()
                if self._options['fc'] == True:
                    modelpath = os.path.join(pwd,'fc_models',(self._options['dataset'] + '_net_params' + '_best_ori_val_exp' + str(self._options['expid']) + '.pkl'))
                else:
                    modelpath = os.path.join(pwd,'db_models',(self._options['dataset'] + '_net_params' + '_best_ori_val_exp' + str(self._options['expid']) + '.pkl'))
        ...
        if self._options['fc'] == True:
            modelpath = os.path.join(pwd,'fc_models',(self._options['dataset'] + '_net_params' + '_best_ori_val_exp' + str(self._options['expid']) + '.pkl')) #CLIVE_net_params _best
        else:
            modelpath = os.path.join(pwd,'db_models',(self._options['dataset'] + '_net_params' + '_best_ori_val_exp' + str(self._options['expid']) + '.pkl'))        
        checkpoint = torch.load(modelpath)
        self._net.load_state_dict(checkpoint)
        print('Load from:', modelpath)
        final_test_srcc, final_test_plcc, final_test_krocc = self._consitency(self._test_loader)
        print('Final test srocc %4.4f, plcc %4.4f, krocc%4.4f' % (final_test_srcc,final_test_plcc,final_test_krocc))
        
        return final_test_srcc#best_srcc
    ...
    def _consitency(self, data_loader):
        ...
        test_krocc, _ = stats.kendalltau(pscores,tscores)
        self._net.train(True)  # Set the model to training phase
        return test_srcc, test_plcc, test_krocc
...
def main():
    ...
    path = {
        ...
        'fc_root': os.path.join('./fc_models',(args.dataset + '_net_params' + '_best_ori_val_exp' + str(options['expid']) + '.pkl')),
    ...
    if options['dataset'] == 'live':          
        import h5py
        datainfo = h5py.File('/YOURPATH/LIVE/LIVEINFO.mat','r')
        ...
        import h5py
        datainfo = h5py.File('/YOURPATH/CLIVEinfo.mat','r')
        random_list = datainfo['index']
        index = random_list[:,options['expid']]
        index = [int(i-1) for i in index]
    ...
    for i in range(0,1): #
        #fixed split train-test set
        print('Fixed index:',index)
        # random.shuffle(index)
        train_index = index[0:round(0.6*len(index))]
        val_index = index[round(0.6*len(index)):round(0.8*len(index))]
        test_index = index[round(0.8*len(index)):len(index)]
    
        options['train_index'] = train_index
        options['val_index'] = val_index
